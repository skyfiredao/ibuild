...
# alias ncstop='nc 127.0.0.1 4444;nc 127.0.0.1 5555'
 ...
 export IBUILD_ID=$(grep '^IBUILD_ID=' $IVERIFY_SPACE/build_info.$SEED | awk -F'IBUILD_ID=' {'print $2'})
     [[ -z $IBUILD_ID ]] && EXIT

 mv $IVERIFY_SPACE/build_info.$SEED $IVERIFY_SPACE/$IBUILD_ID.build_info >/dev/null 2>&1 
 export IBUILD_TARGET_PRODUCT=$(grep '^IBUILD_TARGET_PRODUCT=' $IVERIFY_SPACE/$IBUILD_ID.build_info | awk -F'IBUILD_TARGET_PRODUCT=' {'print $2'})
 export ITASK_REV=$(grep '^ITASK_REV=' $IVERIFY_SPACE/$IBUILD_ID.build_info | awk -F'ITASK_REV=' {'print $2'})
 export ITASK_ORDER=$(grep '^ITASK_ORDER=' $IVERIFY_SPACE/$IBUILD_ID.build_info | awk -F'ITASK_ORDER=' {'print $2'} | head -n1)
 [[ ! -z $ITASK_ORDER ]] && export ITASK_TMP=$ITASK_ORDER || export ITASK_TMP=$ITASK_REV
 export NEW_BUILD_INFO_NAME=$IBUILD_ID.$ITASK_TMP.$IBUILD_TARGET_PRODUCT.build_info

 $NETCAT 127.0.0.1 5555
 echo "$NOW|$IBUILD_ID|$HOSTNAME" | $NETCAT -l 5555
 cat $IVERIFY_CONF | egrep 'EMAIL' >>$IVERIFY_SPACE/$IBUILD_ID.build_info
 /bin/mv $IVERIFY_SPACE/$IBUILD_ID.build_info $LOCAL_IVERIFY_QUEUE/$NEW_BUILD_INFO_NAME
