...
[[ $(id ibuild) ]] || sudo useradd -s /bin/bash -m ibuild
[[ ! -d $SRV_SVN_PATH ]] && sudo mkdir -p $SRV_SVN_PATH/{repo,conf} && sudo chown -R $USER $SRV_SVN_PATH
...
if [[ ! -d ~/ibuild.git ]] ; then
    export GIT_SSL_NO_VERIFY=1
    git clone https://github.com/daviding924/ibuild.git ~/ibuild.git
fi

cp -R ~/ibuild.git $IBUILD_SRC_PATH/ibuild
...
mkdir -p $TMP_SVN_PATH/{ibuild,iverify,itask,ichange,ispec}.source
